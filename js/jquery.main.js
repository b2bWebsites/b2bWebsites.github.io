...
// initialize waitlist API
function initWaitlistAPI() {
  let phoneNumber
  let isValidPhoneNumber = false

  jQuery('#phone').on('input', async (e) => {
    const input = jQuery('#phone')
    const button = jQuery('#join-button')

    // https://stackoverflow.com/a/20992157
  ...
  jQuery('#join-button').on('click', async (e) => {
  ...
  jQuery('#code').on('input', async (e) => {
    const input = jQuery('#code')
    const button = jQuery('#confirm-button')

    code = input.val()
    isValidCode = !isNaN(code) && code.length === 5
  ...
  jQuery('#confirm-button').on('click', async (e) => {
    if (isValidCode) {
